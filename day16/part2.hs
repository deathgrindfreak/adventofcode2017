...
-- The dance repeats every 42 dances, so 1,000,000,000 `mod` 42 == 34.
-- Skip the first inital state, so index at 35
main :: IO ()
main = do
  moves <- parseDance <$> readFile "input"
  print . (!! 35) . iterate (flip dance moves) . S.fromList $ ['a'..'p']
